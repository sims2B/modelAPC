...
sub handle{
	...
	my $cpt = 1;
	foreach (my $inst = 0 ; $inst < 50 ; $inst++){
	#	system("mkdir $target/T$cpt ");
	    system("mkdir $target/T$cpt/T$cpt\_big");
	    system("mv $target/instance\_$i[$inst]\_$j[$inst]\_$f[$inst]\_$p[$inst]\_$q[$inst]\_Bthr\_* $target/T$cpt/T$cpt\_big");
	    $cpt = $cpt +1;
	    }

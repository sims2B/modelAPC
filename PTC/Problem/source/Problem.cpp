...
void generateThreshold(Problem& P/*, const int& n*/, const int& m, const int& F, const int & pmax, const int& smax){
  ...
  for (int f = 0 ; f < F ; ++f){
    ...
    if (sample < problem.getDuration(f) + smax + sizeMin * pmax + problem.getSetup(f))
      problem.getThreshold(f) = problem.getDuration(f) + smax + sizeMin * pmax + problem.getSetup(f);
    if (sample > problem.getDuration(f) + smax + (sizeMin+1) * pmax + problem.getSetup(f))
      problem.getThreshold(f) =  problem.getDuration(f)  + smax + (sizeMin + 1) * pmax + problem.getSetup(f);
    else problem.getThreshold(f) = sample;
  }
 }
...
void generateQualif(Problem& P, const int& m, const int& F, int sumQualif){
  ...
  for (f = 0 ; f < F ; ++f){
    ...
    while (nbQualif[f] > 0){
      ...
      int select = -1;
      while (getIndex > -1 ){
	if (!problem.F[f].qualif[select+1])
	  getIndex--;
	select++;
	select=select% m;
      } 
      problem.F[f].qualif[select] = 1;
      j--; nbQualif[f]--;
    }
  }
}
...
void affectFamily(Problem& P, const int& n, const int& F){
  ...
  for (f = 0 ; f < F ; ++f){
    while (nF[f] > 0){
      int getIndex = (int)(famAffect(generator) % nbRes);
      int select = -1;
      while (getIndex > -1){
...
void generateSetup(Problem& P, const int& F){
  ...
  for (int  f = 0 ; f < F ; ++f){
    problem.getSetup(f) = disSetup(generator);
  }  
}
...
void generateThreshold(Problem& P,  const int& m, const int& F){
  ...
  for (int f = 0 ; f < F ; ++f){
    ...
    if (sample < sizeMin * Pmax + Smax + problem.getSetup(f))
      problem.getThreshold(f) = sizeMin * Pmax + Smax + problem.getSetup(f);
    if (sample > (sizeMin+1) * Pmax + Smax + problem.getSetup(f))
      problem.getThreshold(f) = (sizeMin+1) * Pmax + Smax + problem.getSetup(f);
    else problem.getThreshold(f) = sample;
  }
}
...
void generateDuration(Problem& P,const int& F){
  ...
  for (int  f = 0 ; f < F ; ++f){
    problem.getDuration(f) = disDur(generator);
  }
}
...
Problem readFromFile(std::ifstream& in){
  ...
  for (i = 0 ; i < N ; ++i)
    in >> problem.famOf[i] ;
  for (i = 0 ; i < F ; ++i)
    problem.F[i] = readFamily(in,M);
  std::sort(problem.famOf.begin(),problem.famOf.end());
  return P;
}

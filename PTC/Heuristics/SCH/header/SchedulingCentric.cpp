...
int LH(Problem P, Solution& s){
  ...
  //qualifLostTime is used to store the last occurence of a family on a machine. 
  for ( f = 0 ; f < F ; ++f)
    for (i = 0 ; i < P.M ; ++i)
      if (P.F[f].qualif[i])
...
int getObjectiveValue(const Problem &P, const Solution& s){
...
int chooseFamily(const Problem &P, int m, std::vector<int> toSchedule){
  ...
  for (int f = 0 ; f < P.getFamilyNumber() ; ++f){
    if (toSchedule[f] != 0 && P.F[f].qualif[m]){

...
int CpoSolver1APC::modelToSol(const IloCP& cp,
...
int CpoSolver1APC::solToModel(IloIntervalVarArray& masterTask, IloIntervalVarMatrix& altTasks,
	       IloIntervalVarMatrix& disqualif, IloIntervalVar& Cmax,
	       IloSolution& sol){
  const int F = problem.getFamilyNumber();
  const int M = problem.M;
...
int CpoSolver1APC::createModel(IloEnv& env, IloModel& model,
		IloIntervalVarArray& masterTask, IloIntervalVarMatrix& altTasks,
		IloIntervalVarMatrix& disqualif, IloIntervalSequenceVarArray& mchs){
  return createVariables(env, masterTask, altTasks, disqualif, mchs)
    || createConstraints(env, model, masterTask, altTasks, disqualif, mchs)
    || createObjective(env, model, masterTask, disqualif);
}

int CpoSolver1APC::createVariables(IloEnv& env, IloIntervalVarArray& masterTask,
...
int CpoSolver1APC::createObjective(IloEnv& env, IloModel& model,
...
int CpoSolver1APC::createConstraints(IloEnv& env, IloModel& model,
		      IloIntervalVarArray& masterTask, IloIntervalVarMatrix& altTasks,
		      IloIntervalVarMatrix& disqualif, IloIntervalSequenceVarArray& mchs){
  int i, j, f;
  const int n = problem.N;

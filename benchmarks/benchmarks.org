...
* STARTED Read and summarize results 
 ...
 1      20  100 96.8  2.7  323.6  4.8  70492.2
 2      30   98 68.6  4.4  813.6 15.1 151600.8
 3      40   98 43.1  4.6 1348.7 21.5 172846.7

 $with1SF
   Group.1 feas  opt disq   flow time     fail
 1      20  100 82.3  2.8  323.8  9.4 121280.5
 2      30   98 51.0  4.3  814.4 18.0 180067.0
 3      40   98 39.2  4.7 1351.6 27.8 174951.8

 $without1SF
   Group.1  feas  opt disq   flow time     fail
 1      20 100.0 85.5  2.8  323.7  9.2 121321.5
 2      30  96.1 37.3  4.2  815.7 21.0 194965.2
 3      40  96.1 35.3  4.7 1343.3 22.7 171643.3
 #+end_example
  ...
  if(n > 1) {
    for(i in 1:(n-1)) {
      for(j in (i+1):n) {
        ...
        ggplot(p, aes_string(x="Time", y="Fails", shape=namei, color=namej)) +   geom_point(size=5) + scale_x_continuous(trans='log2') +   scale_y_continuous(trans='log2') + ggtitle(sprintf('%s versus %s', namei, namej)) +  theme(text = element_text(size=20))
